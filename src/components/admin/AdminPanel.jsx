  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'var(--bgp-bg)', zIndex:2500, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'0 20px', height:56, background:'var(--bgp-navy)', borderBottom:'2px solid var(--bgp-teal)', flexShrink:0 }}>
        ...
        <div style={{ flex:1 }} />
        <button onClick={lock} style={{ fontSize:11, padding:'5px 12px', borderRadius:5, border:'1px solid rgba(255,255,255,.2)', background:'transparent', color:'rgba(255,255,255,.6)', cursor:'pointer' }}>🔒 Lock</button>
        <button onClick={() => closeAdmin()} style={{ width:32, height:32, border:'1px solid rgba(255,255,255,.2)', background:'transparent', color:'rgba(255,255,255,.5)', cursor:'pointer', borderRadius:6, fontSize:18, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
      </div>
